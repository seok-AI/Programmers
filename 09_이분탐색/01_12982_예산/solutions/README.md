...
| 파일 | 접근 | 복잡도 | 용도 |
|---|---|---|---|
| [효율적인 권장 풀이](./recommended.py) | 신청 금액을 작은 순서로 더해 예산 내 부서 수 최대화 | `O(n log n) time, O(n) space` | 실전 제출의 기준 풀이 |
| [공개 풀이 접근(재작성)](./alternative.py) | 정렬한 금액의 누적합에서 예산의 삽입 위치 탐색 | `O(n log n) time, O(n) space` | 같은 문제를 다른 상태 표현으로 비교하는 풀이 |

## 풀이별 해설

### `recommended.py` — 효율적인 권장 풀이

- 핵심 사고: 신청 금액을 작은 순서로 더해 예산 내 부서 수 최대화
...
- 핵심 사고: 정렬한 금액의 누적합에서 예산의 삽입 위치 탐색
- 복잡도: `O(n log n) time, O(n) space`
- 적용 범위: 문제의 최대 제한에서도 실행 가능한 대안
- 문제별 주의: 지원 금액을 일부만 줄 수 없으므로 다음 최솟값도 못 사면 이후 값도 모두 못 산다.
...
- [프로그래머스 문제별 풀이 Q&A](https://school.programmers.co.kr/learn/courses/30/lessons/12982/questions)

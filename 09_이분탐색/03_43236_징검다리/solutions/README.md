...
| 파일 | 접근 | 복잡도 | 용도 |
|---|---|---|---|
| [효율적인 권장 풀이](./recommended.py) | 최소 거리의 가능 여부를 판정하는 매개변수 이분탐색 | `O(n log distance) time, O(n) space` | 실전 제출의 기준 풀이 |
| [공개 풀이 접근(재작성)](./alternative.py) | 제거 횟수를 직접 세며 가능한 거리의 최댓값 저장 | `O(n log distance) time, O(n) space` | 같은 문제를 다른 상태 표현으로 비교하는 풀이 |

## 풀이별 해설

### `recommended.py` — 효율적인 권장 풀이

- 핵심 사고: 최소 거리의 가능 여부를 판정하는 매개변수 이분탐색
...
- 핵심 사고: 제거 횟수를 직접 세며 가능한 거리의 최댓값 저장
- 복잡도: `O(n log distance) time, O(n) space`
- 적용 범위: 문제의 최대 제한에서도 실행 가능한 대안
- 문제별 주의: 마지막 바위와 도착점 사이 거리도 판정에 반드시 포함한다.
...
- [프로그래머스 문제별 풀이 Q&A](https://school.programmers.co.kr/learn/courses/30/lessons/43236/questions)

...
| 파일 | 접근 | 복잡도 | 용도 |
|---|---|---|---|
| [효율적인 권장 풀이](./recommended.py) | 주어진 시간에 처리 가능한 사람 수로 최소 시간 이분탐색 | `O(k log answer) time, O(1) space` | 실전 제출의 기준 풀이 |
| [공개 풀이 접근(재작성)](./alternative.py) | 첫 가능 시간을 찾는 닫힌 구간 이분탐색 | `O(k log answer) time, O(1) space` | 같은 문제를 다른 상태 표현으로 비교하는 풀이 |

## 풀이별 해설

### `recommended.py` — 효율적인 권장 풀이

- 핵심 사고: 주어진 시간에 처리 가능한 사람 수로 최소 시간 이분탐색
...
- 핵심 사고: 첫 가능 시간을 찾는 닫힌 구간 이분탐색
- 복잡도: `O(k log answer) time, O(1) space`
- 적용 범위: 문제의 최대 제한에서도 실행 가능한 대안
- 문제별 주의: 구하려는 것은 정확히 n명을 처리하는 시간이 아니라 n명 이상 가능한 최초 시간이다.
...
- [프로그래머스 문제별 풀이 Q&A](https://school.programmers.co.kr/learn/courses/30/lessons/43238/questions)

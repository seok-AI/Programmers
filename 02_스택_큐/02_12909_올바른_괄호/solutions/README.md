...
| 파일 | 접근 | 복잡도 | 용도 |
|---|---|---|---|
| [효율적인 권장 풀이](./recommended.py) | 접두 구간의 괄호 균형 유지 | `O(n) time, O(1) space` | 실전 제출의 기준 풀이 |
| [공개 풀이 접근(재작성)](./alternative.py) | 여는 괄호를 스택에 저장 | `O(n) time, O(n) space` | 같은 문제를 다른 상태 표현으로 비교하는 풀이 |

## 풀이별 해설

### `recommended.py` — 효율적인 권장 풀이

- 핵심 사고: 접두 구간의 괄호 균형 유지
...
- 문제별 주의: 최종 개수만 같아도 닫는 괄호가 먼저 나온 문자열은 올바르지 않다.
...
- [프로그래머스 문제별 풀이 Q&A](https://school.programmers.co.kr/learn/courses/30/lessons/12909/questions)

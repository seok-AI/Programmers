...
| 파일 | 접근 | 복잡도 | 용도 |
|---|---|---|---|
| [효율적인 권장 풀이](./recommended.py) | 가격이 처음 떨어지는 시점을 단조 스택으로 확정 | `O(n) time, O(n) space` | 실전 제출의 기준 풀이 |
| [공개 풀이 접근(재작성)](./alternative.py) | 기본 지속 시간을 채운 뒤 단조 스택으로 하락 시점 갱신 | `O(n) time, O(n) space` | 같은 문제를 다른 상태 표현으로 비교하는 풀이 |

## 풀이별 해설

### `recommended.py` — 효율적인 권장 풀이

- 핵심 사고: 가격이 처음 떨어지는 시점을 단조 스택으로 확정
...
- 핵심 사고: 기본 지속 시간을 채운 뒤 단조 스택으로 하락 시점 갱신
- 복잡도: `O(n) time, O(n) space`
- 적용 범위: 문제의 최대 제한에서도 실행 가능한 대안
- 문제별 주의: 가격이 같은 것은 하락이 아니므로 비교 연산은 엄격한 큰 값 조건이어야 한다.
...
- [프로그래머스 문제별 풀이 Q&A](https://school.programmers.co.kr/learn/courses/30/lessons/42584/questions)

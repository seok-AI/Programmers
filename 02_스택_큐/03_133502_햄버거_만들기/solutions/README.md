...
| 파일 | 접근 | 복잡도 | 용도 |
|---|---|---|---|
| [효율적인 권장 풀이](./recommended.py) | 최근 네 재료를 스택에서 즉시 제거 | `O(n) time, O(n) space` | 실전 제출의 기준 풀이 |
| [공개 풀이 접근(재작성)](./alternative.py) | 고정 배열과 top 포인터로 스택 구현 | `O(n) time, O(n) space` | 같은 문제를 다른 상태 표현으로 비교하는 풀이 |

## 풀이별 해설

### `recommended.py` — 효율적인 권장 풀이

- 핵심 사고: 최근 네 재료를 스택에서 즉시 제거
...
- 핵심 사고: 고정 배열과 top 포인터로 스택 구현
- 복잡도: `O(n) time, O(n) space`
- 적용 범위: 문제의 최대 제한에서도 실행 가능한 대안
- 문제별 주의: 패턴 제거 뒤 생기는 새 경계는 다음 재료 처리 시 스택 끝에서 자연스럽게 검사된다.
...
- [프로그래머스 문제별 풀이 Q&A](https://school.programmers.co.kr/learn/courses/30/lessons/133502/questions)

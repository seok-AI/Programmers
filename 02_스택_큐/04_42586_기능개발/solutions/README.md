...
| 파일 | 접근 | 복잡도 | 용도 |
|---|---|---|---|
| [효율적인 권장 풀이](./recommended.py) | 기능별 완료일 계산 후 비감소 배포 경계 그룹화 | `O(n) time, O(n) space` | 실전 제출의 기준 풀이 |
| [공개 풀이 접근(재작성)](./alternative.py) | 완료일 큐에서 선두 이하를 묶어 배포 | `O(n) time, O(n) space` | 같은 문제를 다른 상태 표현으로 비교하는 풀이 |

## 풀이별 해설

### `recommended.py` — 효율적인 권장 풀이

- 핵심 사고: 기능별 완료일 계산 후 비감소 배포 경계 그룹화
...
- 핵심 사고: 완료일 큐에서 선두 이하를 묶어 배포
- 복잡도: `O(n) time, O(n) space`
- 적용 범위: 문제의 최대 제한에서도 실행 가능한 대안
- 문제별 주의: 뒤 기능이 먼저 끝나도 앞 기능을 건너뛰어 배포할 수 없다.
...
- [프로그래머스 문제별 풀이 Q&A](https://school.programmers.co.kr/learn/courses/30/lessons/42586/questions)

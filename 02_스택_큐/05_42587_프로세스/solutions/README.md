...
| 파일 | 접근 | 복잡도 | 용도 |
|---|---|---|---|
| [효율적인 권장 풀이](./recommended.py) | 큐 순환과 남은 우선순위 빈도표 | `O(n) time, O(n) space` | 실전 제출의 기준 풀이 |
| [공개 풀이 접근(재작성)](./alternative.py) | 매 회차 큐의 최대 우선순위와 비교 | `O(n^2) time, O(n) space` | n<=100에서 충분한 단순 풀이 |

## 풀이별 해설

### `recommended.py` — 효율적인 권장 풀이

- 핵심 사고: 큐 순환과 남은 우선순위 빈도표
...
- 핵심 사고: 매 회차 큐의 최대 우선순위와 비교
- 복잡도: `O(n^2) time, O(n) space`
- 적용 범위: n<=100에서 충분한 단순 풀이
- 문제별 주의: 같은 우선순위끼리는 큐의 기존 순서가 보존되어야 한다.
...
- [프로그래머스 문제별 풀이 Q&A](https://school.programmers.co.kr/learn/courses/30/lessons/42587/questions)

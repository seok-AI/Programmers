...
| 파일 | 접근 | 복잡도 | 용도 |
|---|---|---|---|
| [효율적인 권장 풀이](./recommended.py) | 트럭별 진입 시각과 종료 시각을 이벤트로 관리 | `O(n) time, O(n) space` | 실전 제출의 기준 풀이 |
| [공개 풀이 접근(재작성)](./alternative.py) | 다리 길이만큼의 고정 큐를 매초 이동 | `O(total seconds) time, O(bridge_length) space` | 같은 문제를 다른 상태 표현으로 비교하는 풀이 |

## 풀이별 해설

### `recommended.py` — 효율적인 권장 풀이

- 핵심 사고: 트럭별 진입 시각과 종료 시각을 이벤트로 관리
...
- [프로그래머스 문제별 풀이 Q&A](https://school.programmers.co.kr/learn/courses/30/lessons/42583/questions)

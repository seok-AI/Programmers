...
| 파일 | 접근 | 복잡도 | 용도 |
|---|---|---|---|
| [효율적인 권장 풀이](./recommended.py) | 최소 힙에서 가장 작은 두 음식 반복 혼합 | `O(n log n) time, O(n) space` | 실전 제출의 기준 풀이 |
| [공개 풀이 접근(재작성)](./alternative.py) | 정렬 원본 큐와 비감소 생성값 큐 병합 | `O(n log n) time, O(n) space` | 같은 문제를 다른 상태 표현으로 비교하는 풀이 |

## 풀이별 해설

### `recommended.py` — 효율적인 권장 풀이

- 핵심 사고: 최소 힙에서 가장 작은 두 음식 반복 혼합
...
- 핵심 사고: 정렬 원본 큐와 비감소 생성값 큐 병합
- 복잡도: `O(n log n) time, O(n) space`
- 적용 범위: 문제의 최대 제한에서도 실행 가능한 대안
- 문제별 주의: 루트가 미달인데 힙에 원소가 하나뿐이면 더 섞을 수 없으므로 -1이다.
...
- [프로그래머스 문제별 풀이 Q&A](https://school.programmers.co.kr/learn/courses/30/lessons/42626/questions)

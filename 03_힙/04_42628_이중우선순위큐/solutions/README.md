...
| 파일 | 접근 | 복잡도 | 용도 |
|---|---|---|---|
| [효율적인 권장 풀이](./recommended.py) | 최소/최대 힙과 활성 ID로 지연 삭제 | `O(n log n) time, O(n) space` | 실전 제출의 기준 풀이 |
| [공개 풀이 접근(재작성)](./alternative.py) | 두 힙과 값별 개수로 지연 삭제 | `O(n log n) time, O(n) space` | 같은 문제를 다른 상태 표현으로 비교하는 풀이 |

## 풀이별 해설

### `recommended.py` — 효율적인 권장 풀이

- 핵심 사고: 최소/최대 힙과 활성 ID로 지연 삭제
...
- 핵심 사고: 두 힙과 값별 개수로 지연 삭제
- 복잡도: `O(n log n) time, O(n) space`
- 적용 범위: 문제의 최대 제한에서도 실행 가능한 대안
- 문제별 주의: 한 힙에서 삭제했다고 다른 힙의 같은 항목을 즉시 찾으려 하면 선형 시간이 든다.
...
- [프로그래머스 문제별 풀이 Q&A](https://school.programmers.co.kr/learn/courses/30/lessons/42628/questions)

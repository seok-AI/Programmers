...
| 파일 | 접근 | 복잡도 | 용도 |
|---|---|---|---|
| [효율적인 권장 풀이](./recommended.py) | 요청 시각 정렬 + 실행 시간 우선 힙 | `O(n log n) time, O(n) space` | 실전 제출의 기준 풀이 |
| [공개 풀이 접근(재작성)](./alternative.py) | 매 시점 가능한 작업을 선형 선택 | `O(n^2) time, O(n) space` | n<=500에서 이해하기 쉬운 대안 |

## 풀이별 해설

### `recommended.py` — 효율적인 권장 풀이

- 핵심 사고: 요청 시각 정렬 + 실행 시간 우선 힙
...
- 핵심 사고: 매 시점 가능한 작업을 선형 선택
- 복잡도: `O(n^2) time, O(n) space`
- 적용 범위: n<=500에서 이해하기 쉬운 대안
- 문제별 주의: 대기 작업이 없을 때 시각을 1씩 늘리지 말고 다음 요청 시각으로 이동한다.
...
- [프로그래머스 문제별 풀이 Q&A](https://school.programmers.co.kr/learn/courses/30/lessons/42627/questions)

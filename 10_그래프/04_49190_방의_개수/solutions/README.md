...
| 파일 | 접근 | 복잡도 | 용도 |
|---|---|---|---|
| [효율적인 권장 풀이](./recommended.py) | 대각선 교차를 잡도록 이동을 2분할하고 새 간선의 재방문 정점 집계 | `O(a) time, O(a) space` | 실전 제출의 기준 풀이 |
| [공개 풀이 접근(재작성)](./alternative.py) | 확대 경로 그래프의 V-E+F 관계로 방 개수 계산 | `O(a) time, O(a) space` | 같은 문제를 다른 상태 표현으로 비교하는 풀이 |

## 풀이별 해설

### `recommended.py` — 효율적인 권장 풀이

- 핵심 사고: 대각선 교차를 잡도록 이동을 2분할하고 새 간선의 재방문 정점 집계
...
- 핵심 사고: 확대 경로 그래프의 V-E+F 관계로 방 개수 계산
- 복잡도: `O(a) time, O(a) space`
- 적용 범위: 문제의 최대 제한에서도 실행 가능한 대안
- 문제별 주의: 대각선 두 선분의 중간 교차를 정점으로 잡기 위해 이동을 두 배로 세분해야 한다.
...
- [프로그래머스 문제별 풀이 Q&A](https://school.programmers.co.kr/learn/courses/30/lessons/49190/questions)

...
| 파일 | 접근 | 복잡도 | 용도 |
|---|---|---|---|
| [효율적인 권장 풀이](./recommended.py) | Floyd-Warshall로 모든 승패 도달 관계 계산 | `O(n^3) time, O(n^2) space` | 실전 제출의 기준 풀이 |
| [공개 풀이 접근(재작성)](./alternative.py) | 각 선수에서 정방향·역방향 DFS로 비교 가능한 선수 집계 | `O(n*(n+e)) time, O(n+e) space` | 같은 문제를 다른 상태 표현으로 비교하는 풀이 |

## 풀이별 해설

### `recommended.py` — 효율적인 권장 풀이

- 핵심 사고: Floyd-Warshall로 모든 승패 도달 관계 계산
...
- 핵심 사고: 각 선수에서 정방향·역방향 DFS로 비교 가능한 선수 집계
- 복잡도: `O(n*(n+e)) time, O(n+e) space`
- 적용 범위: 문제의 최대 제한에서도 실행 가능한 대안
- 문제별 주의: A가 B를 이겼다면 B의 패배 관계도 같은 사실이므로 순위 확정 시 양방향 중 하나를 확인한다.
...
- [프로그래머스 문제별 풀이 Q&A](https://school.programmers.co.kr/learn/courses/30/lessons/49191/questions)

...
| 파일 | 접근 | 복잡도 | 용도 |
|---|---|---|---|
| [효율적인 권장 풀이](./recommended.py) | 키패드 좌표의 맨해튼 거리와 주손 우선순위 비교 | `O(n) time, O(1) space` | 실전 제출의 기준 풀이 |
| [공개 풀이 접근(재작성)](./alternative.py) | 키패드 인덱스의 행·열 차이로 거리 계산 | `O(n) time, O(1) space` | 같은 문제를 다른 상태 표현으로 비교하는 풀이 |

## 풀이별 해설

### `recommended.py` — 효율적인 권장 풀이

- 핵심 사고: 키패드 좌표의 맨해튼 거리와 주손 우선순위 비교
...
- 핵심 사고: 키패드 인덱스의 행·열 차이로 거리 계산
- 복잡도: `O(n) time, O(1) space`
- 적용 범위: 문제의 최대 제한에서도 실행 가능한 대안
- 문제별 주의: 0의 좌표와 시작 위치 *·#를 포함한 거리 계산을 정확히 정의한다.
...
- [프로그래머스 문제별 풀이 Q&A](https://school.programmers.co.kr/learn/courses/30/lessons/67256/questions)

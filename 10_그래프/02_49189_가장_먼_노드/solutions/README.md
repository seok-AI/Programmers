...
| 파일 | 접근 | 복잡도 | 용도 |
|---|---|---|---|
| [효율적인 권장 풀이](./recommended.py) | 1번 노드부터 BFS 거리 계산 후 최댓값 빈도 집계 | `O(n+e) time, O(n+e) space` | 실전 제출의 기준 풀이 |
| [공개 풀이 접근(재작성)](./alternative.py) | BFS의 현재 레이어 집합을 교체하며 마지막 레이어 크기 반환 | `O(n+e) time, O(n+e) space` | 같은 문제를 다른 상태 표현으로 비교하는 풀이 |

## 풀이별 해설

### `recommended.py` — 효율적인 권장 풀이

- 핵심 사고: 1번 노드부터 BFS 거리 계산 후 최댓값 빈도 집계
...
- 핵심 사고: BFS의 현재 레이어 집합을 교체하며 마지막 레이어 크기 반환
- 복잡도: `O(n+e) time, O(n+e) space`
- 적용 범위: 문제의 최대 제한에서도 실행 가능한 대안
- 문제별 주의: 그래프는 무방향이므로 각 간선을 양쪽 인접 리스트에 넣는다.
...
- [프로그래머스 문제별 풀이 Q&A](https://school.programmers.co.kr/learn/courses/30/lessons/49189/questions)

...
| 파일 | 접근 | 복잡도 | 용도 |
|---|---|---|---|
| [효율적인 권장 풀이](./recommended.py) | 정렬한 인용수와 남은 논문 수 비교 | `O(n log n) time, O(n) space` | 실전 제출의 기준 풀이 |
| [공개 풀이 접근(재작성)](./alternative.py) | 가능한 h를 큰 값부터 직접 검증 | `O(n^2) time, O(1) space` | n<=1,000에서 가능한 정의 그대로의 풀이 |

## 풀이별 해설

### `recommended.py` — 효율적인 권장 풀이

- 핵심 사고: 정렬한 인용수와 남은 논문 수 비교
...
- 핵심 사고: 가능한 h를 큰 값부터 직접 검증
- 복잡도: `O(n^2) time, O(1) space`
- 적용 범위: n<=1,000에서 가능한 정의 그대로의 풀이
- 문제별 주의: H-index는 배열에 실제로 존재하는 인용 수일 필요가 없다.
...
- [프로그래머스 문제별 풀이 Q&A](https://school.programmers.co.kr/learn/courses/30/lessons/42747/questions)

...
| 파일 | 접근 | 복잡도 | 용도 |
|---|---|---|---|
| [효율적인 권장 풀이](./recommended.py) | 명령별 슬라이스 정렬 | `O(q*k log k) time, O(k) space` | 실전 제출의 기준 풀이 |
| [공개 풀이 접근(재작성)](./alternative.py) | 반복문으로 1기반 명령을 명시적으로 변환 | `O(q*k log k) time, O(k) space` | 같은 문제를 다른 상태 표현으로 비교하는 풀이 |
...
- 핵심 사고: 반복문으로 1기반 명령을 명시적으로 변환
- 복잡도: `O(q*k log k) time, O(k) space`
- 적용 범위: 문제의 최대 제한에서도 실행 가능한 대안
- 문제별 주의: 문제의 i, j, k는 1부터 세지만 Python 인덱스는 0부터 센다.
...
- [프로그래머스 문제별 풀이 Q&A](https://school.programmers.co.kr/learn/courses/30/lessons/42748/questions)

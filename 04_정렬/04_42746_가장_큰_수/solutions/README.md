...
| 파일 | 접근 | 복잡도 | 용도 |
|---|---|---|---|
| [효율적인 권장 풀이](./recommended.py) | a+b와 b+a를 비교하는 사용자 정렬 | `O(n log n) time, O(n) space` | 실전 제출의 기준 풀이 |
| [공개 풀이 접근(재작성)](./alternative.py) | 최대 4자리 반복 키로 내림차순 정렬 | `O(n log n) time, O(n) space` | 각 수가 0..1000인 제한을 이용 |

## 풀이별 해설

### `recommended.py` — 효율적인 권장 풀이

- 핵심 사고: a+b와 b+a를 비교하는 사용자 정렬
...
- 핵심 사고: 최대 4자리 반복 키로 내림차순 정렬
- 복잡도: `O(n log n) time, O(n) space`
- 적용 범위: 각 수가 0..1000인 제한을 이용
- 문제별 주의: 숫자 크기나 문자열 사전순만으로 정렬할 수 없고, 전부 0인 결과도 정규화해야 한다.
...
- [프로그래머스 문제별 풀이 Q&A](https://school.programmers.co.kr/learn/courses/30/lessons/42746/questions)

...
| 파일 | 접근 | 복잡도 | 용도 |
|---|---|---|---|
| [효율적인 권장 풀이](./recommended.py) | 빈 칸으로 끝나는 최대 정사각형 DP 후 돗자리 선택 | `O(h*w + m log m) time, O(w) space` | 실전 제출의 기준 풀이 |
| [공개 풀이 접근(재작성)](./alternative.py) | 2차원 누적합으로 큰 돗자리부터 빈 영역 탐색 | `O(h*w + m*h*w) time, O(h*w) space` | 돗자리 종류가 10개 이하인 제한을 이용 |

## 풀이별 해설

### `recommended.py` — 효율적인 권장 풀이

- 핵심 사고: 빈 칸으로 끝나는 최대 정사각형 DP 후 돗자리 선택
...
- 핵심 사고: 2차원 누적합으로 큰 돗자리부터 빈 영역 탐색
- 복잡도: `O(h*w + m*h*w) time, O(h*w) space`
- 적용 범위: 돗자리 종류가 10개 이하인 제한을 이용
- 문제별 주의: 문자열 '-1'이 빈 칸이며, 돗자리 목록에 맞는 크기가 없으면 -1이다.
...
- [프로그래머스 문제별 풀이 Q&A](https://school.programmers.co.kr/learn/courses/30/lessons/340198/questions)

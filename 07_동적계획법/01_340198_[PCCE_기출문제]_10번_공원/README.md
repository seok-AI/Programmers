...
- 함수: `solution(mats, park)`
- 포함된 테스트: 공식 공개 예제 1개 + 자체 경계 사례 2개

## 이 유형에 넣은 이유

빈칸 최대 정사각형을 2차원 DP로도 찾을 수 있어 재분류했습니다. 다만 원래
제한에서는 돗자리 크기를 직접 검사하는 구현이 더 자연스러우므로 DP 대표 문제로
오해하지 말고 `정수 삼각형`부터 핵심 DP를 익히세요.
...
```bash
python3 tools/judge.py "07_동적계획법/01_340198_[PCCE_기출문제]_10번_공원"
```

채점기는 `mats`, `park` 순서로 함수를 호출합니다. `tests.json`은 공개 예제,
...
```bash
python3 "07_동적계획법/01_340198_[PCCE_기출문제]_10번_공원/solution.py"
```

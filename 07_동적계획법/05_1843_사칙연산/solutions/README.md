...
| 파일 | 접근 | 복잡도 | 용도 |
|---|---|---|---|
| [효율적인 권장 풀이](./recommended.py) | 구간별 최솟값·최댓값을 함께 저장하는 구간 DP | `O(n^3) time, O(n^2) space` | 실전 제출의 기준 풀이 |
| [공개 풀이 접근(재작성)](./alternative.py) | 구간의 최솟값·최댓값을 재귀 메모이제이션 | `O(n^3) time, O(n^2) space` | 같은 문제를 다른 상태 표현으로 비교하는 풀이 |

## 풀이별 해설

### `recommended.py` — 효율적인 권장 풀이

- 핵심 사고: 구간별 최솟값·최댓값을 함께 저장하는 구간 DP
...
- 핵심 사고: 구간의 최솟값·최댓값을 재귀 메모이제이션
- 복잡도: `O(n^3) time, O(n^2) space`
- 적용 범위: 문제의 최대 제한에서도 실행 가능한 대안
- 문제별 주의: 뺄셈의 최댓값은 왼쪽 최대-오른쪽 최소이므로 최댓값만 저장해서는 안 된다.
...
- [프로그래머스 문제별 풀이 Q&A](https://school.programmers.co.kr/learn/courses/30/lessons/1843/questions)

...
| 파일 | 접근 | 복잡도 | 용도 |
|---|---|---|---|
| [효율적인 권장 풀이](./recommended.py) | 행을 순회하며 왼쪽·위쪽 경로 수를 1차원 DP에 누적 | `O(m*n) time, O(m) space` | 실전 제출의 기준 풀이 |
| [공개 풀이 접근(재작성)](./alternative.py) | 격자 모양 2차원 DP로 경로 수 계산 | `O(m*n) time, O(m*n) space` | 같은 문제를 다른 상태 표현으로 비교하는 풀이 |

## 풀이별 해설

### `recommended.py` — 효율적인 권장 풀이

- 핵심 사고: 행을 순회하며 왼쪽·위쪽 경로 수를 1차원 DP에 누적
...
- 핵심 사고: 격자 모양 2차원 DP로 경로 수 계산
- 복잡도: `O(m*n) time, O(m*n) space`
- 적용 범위: 문제의 최대 제한에서도 실행 가능한 대안
- 문제별 주의: 물웅덩이 좌표는 [열, 행] 순서라 행렬 인덱스와 반대이다.
...
- [프로그래머스 문제별 풀이 Q&A](https://school.programmers.co.kr/learn/courses/30/lessons/42898/questions)

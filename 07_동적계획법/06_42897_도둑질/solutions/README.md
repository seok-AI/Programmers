...
| 파일 | 접근 | 복잡도 | 용도 |
|---|---|---|---|
| [효율적인 권장 풀이](./recommended.py) | 첫 집 포함/제외를 나눠 선형 도둑질 DP 두 번 | `O(n) time, O(1) extra space` | 실전 제출의 기준 풀이 |
| [공개 풀이 접근(재작성)](./alternative.py) | 첫 집을 고르는 경우와 버리는 경우의 DP 배열 | `O(n) time, O(n) space` | 같은 문제를 다른 상태 표현으로 비교하는 풀이 |

## 풀이별 해설

### `recommended.py` — 효율적인 권장 풀이

- 핵심 사고: 첫 집 포함/제외를 나눠 선형 도둑질 DP 두 번
...
- 핵심 사고: 첫 집을 고르는 경우와 버리는 경우의 DP 배열
- 복잡도: `O(n) time, O(n) space`
- 적용 범위: 문제의 최대 제한에서도 실행 가능한 대안
- 문제별 주의: 원형에서는 첫 집과 마지막 집을 동시에 털 수 없으므로 한 번의 선형 DP로 풀 수 없다.
...
- [프로그래머스 문제별 풀이 Q&A](https://school.programmers.co.kr/learn/courses/30/lessons/42897/questions)

...
| 파일 | 접근 | 복잡도 | 용도 |
|---|---|---|---|
| [효율적인 권장 풀이](./recommended.py) | 사용 횟수별 만들 수 있는 수의 집합 DP | `O(8^3*S^2) worst time, O(8*S) space` | 실전 제출의 기준 풀이 |
| [공개 풀이 접근(재작성)](./alternative.py) | 필요 개수 집합을 재귀적으로 조합해 메모이제이션 | `O(8^3*S^2) worst time, O(8*S) space` | 같은 문제를 다른 상태 표현으로 비교하는 풀이 |

## 풀이별 해설

### `recommended.py` — 효율적인 권장 풀이

- 핵심 사고: 사용 횟수별 만들 수 있는 수의 집합 DP
...
- 핵심 사고: 필요 개수 집합을 재귀적으로 조합해 메모이제이션
- 복잡도: `O(8^3*S^2) worst time, O(8*S) space`
- 적용 범위: 문제의 최대 제한에서도 실행 가능한 대안
- 문제별 주의: 뺄셈·나눗셈은 순서가 중요하고 0으로 나누는 경우는 제외한다.
...
- [프로그래머스 문제별 풀이 Q&A](https://school.programmers.co.kr/learn/courses/30/lessons/42895/questions)

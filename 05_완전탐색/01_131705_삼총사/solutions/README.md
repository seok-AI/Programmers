...
| 파일 | 접근 | 복잡도 | 용도 |
|---|---|---|---|
| [효율적인 권장 풀이](./recommended.py) | 세 수의 조합을 열거해 합이 0인지 확인 | `O(n^3) time, O(1) extra space` | 실전 제출의 기준 풀이 |
| [공개 풀이 접근(재작성)](./alternative.py) | 서로 다른 세 인덱스를 삼중 반복문으로 선택 | `O(n^3) time, O(1) space` | 같은 문제를 다른 상태 표현으로 비교하는 풀이 |

## 풀이별 해설

### `recommended.py` — 효율적인 권장 풀이

- 핵심 사고: 세 수의 조합을 열거해 합이 0인지 확인
...
- 핵심 사고: 서로 다른 세 인덱스를 삼중 반복문으로 선택
- 복잡도: `O(n^3) time, O(1) space`
- 적용 범위: 문제의 최대 제한에서도 실행 가능한 대안
- 문제별 주의: 같은 값이 여러 번 있어도 서로 다른 인덱스 조합은 각각 세어야 한다.
...
- [프로그래머스 문제별 풀이 Q&A](https://school.programmers.co.kr/learn/courses/30/lessons/131705/questions)

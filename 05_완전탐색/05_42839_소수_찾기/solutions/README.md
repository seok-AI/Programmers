...
| 파일 | 접근 | 복잡도 | 용도 |
|---|---|---|---|
| [효율적인 권장 풀이](./recommended.py) | 모든 숫자 순열을 집합에 모아 소수 판정 | `O(sum P(n,k) * sqrt(M)) time, O(sum P(n,k)) space` | 실전 제출의 기준 풀이 |
| [공개 풀이 접근(재작성)](./alternative.py) | DFS로 수를 만들고 에라토스테네스의 체로 일괄 판정 | `O(n*n! + M log log M) time, O(M) space` | 같은 문제를 다른 상태 표현으로 비교하는 풀이 |

## 풀이별 해설

### `recommended.py` — 효율적인 권장 풀이

- 핵심 사고: 모든 숫자 순열을 집합에 모아 소수 판정
...
- 핵심 사고: DFS로 수를 만들고 에라토스테네스의 체로 일괄 판정
- 복잡도: `O(n*n! + M log log M) time, O(M) space`
- 적용 범위: 문제의 최대 제한에서도 실행 가능한 대안
- 문제별 주의: 0과 1은 소수가 아니며 약수 검사는 후보의 제곱근까지만 하면 된다.
...
- [프로그래머스 문제별 풀이 Q&A](https://school.programmers.co.kr/learn/courses/30/lessons/42839/questions)

...
| 파일 | 접근 | 복잡도 | 용도 |
|---|---|---|---|
| [효율적인 권장 풀이](./recommended.py) | 간선을 하나씩 끊고 BFS로 한쪽 송전탑 수 계산 | `O(n^2) time, O(n) space` | 실전 제출의 기준 풀이 |
| [공개 풀이 접근(재작성)](./alternative.py) | 트리를 한 번 순회해 각 자식 서브트리 크기 계산 | `O(n) time, O(n) space` | 같은 문제를 다른 상태 표현으로 비교하는 풀이 |

## 풀이별 해설

### `recommended.py` — 효율적인 권장 풀이

- 핵심 사고: 간선을 하나씩 끊고 BFS로 한쪽 송전탑 수 계산
...
- 핵심 사고: 트리를 한 번 순회해 각 자식 서브트리 크기 계산
- 복잡도: `O(n) time, O(n) space`
- 적용 범위: 문제의 최대 제한에서도 실행 가능한 대안
- 문제별 주의: 입력이 트리라는 보장 덕분에 한쪽 크기만 알면 다른 쪽은 n-size이다.
...
- [프로그래머스 문제별 풀이 Q&A](https://school.programmers.co.kr/learn/courses/30/lessons/86971/questions)

...
| 파일 | 접근 | 복잡도 | 용도 |
|---|---|---|---|
| [효율적인 권장 풀이](./recommended.py) | 각 자리의 사전순 가중치로 순번을 바로 계산 | `O(5) time, O(1) space` | 실전 제출의 기준 풀이 |
| [공개 풀이 접근(재작성)](./alternative.py) | 길이 1..5의 모든 단어를 생성해 사전순 위치 탐색 | `O(5^5 log 5^5) time, O(5^5) space` | 전체 단어가 3,905개뿐인 제한을 이용 |

## 풀이별 해설

### `recommended.py` — 효율적인 권장 풀이

- 핵심 사고: 각 자리의 사전순 가중치로 순번을 바로 계산
...
- 핵심 사고: 길이 1..5의 모든 단어를 생성해 사전순 위치 탐색
- 복잡도: `O(5^5 log 5^5) time, O(5^5) space`
- 적용 범위: 전체 단어가 3,905개뿐인 제한을 이용
- 문제별 주의: 문자 블록뿐 아니라 각 길이의 접두어 단어 자체 때문에 자리마다 1을 더한다.
...
- [프로그래머스 문제별 풀이 Q&A](https://school.programmers.co.kr/learn/courses/30/lessons/84512/questions)

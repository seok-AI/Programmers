...
| 파일 | 접근 | 복잡도 | 용도 |
|---|---|---|---|
| [효율적인 권장 풀이](./recommended.py) | 전체 격자의 약수 쌍에서 테두리 조건 확인 | `O(sqrt(area)) time, O(1) space` | 실전 제출의 기준 풀이 |
| [공개 풀이 접근(재작성)](./alternative.py) | 노란 내부 높이를 늘리며 가로 길이 계산 | `O(sqrt(yellow)) time, O(1) space` | 같은 문제를 다른 상태 표현으로 비교하는 풀이 |

## 풀이별 해설

### `recommended.py` — 효율적인 권장 풀이

- 핵심 사고: 전체 격자의 약수 쌍에서 테두리 조건 확인
...
- 핵심 사고: 노란 내부 높이를 늘리며 가로 길이 계산
- 복잡도: `O(sqrt(yellow)) time, O(1) space`
- 적용 범위: 문제의 최대 제한에서도 실행 가능한 대안
- 문제별 주의: 가로가 세로 이상이어야 하며 테두리 두께 1 때문에 각 변에서 2를 뺀다.
...
- [프로그래머스 문제별 풀이 Q&A](https://school.programmers.co.kr/learn/courses/30/lessons/42842/questions)

...
| 파일 | 접근 | 복잡도 | 용도 |
|---|---|---|---|
| [효율적인 권장 풀이](./recommended.py) | 각 명함의 긴 변과 짧은 변을 같은 축으로 정렬 | `O(n) time, O(1) space` | 실전 제출의 기준 풀이 |
| [공개 풀이 접근(재작성)](./alternative.py) | 필요할 때만 명함을 회전하며 지갑 치수 갱신 | `O(n) time, O(1) space` | 같은 문제를 다른 상태 표현으로 비교하는 풀이 |

## 풀이별 해설

### `recommended.py` — 효율적인 권장 풀이

- 핵심 사고: 각 명함의 긴 변과 짧은 변을 같은 축으로 정렬
...
- 핵심 사고: 필요할 때만 명함을 회전하며 지갑 치수 갱신
- 복잡도: `O(n) time, O(1) space`
- 적용 범위: 문제의 최대 제한에서도 실행 가능한 대안
- 문제별 주의: 명함마다 독립적으로 회전할 수 있으므로 원래 가로·세로 축에 묶이지 않는다.
...
- [프로그래머스 문제별 풀이 Q&A](https://school.programmers.co.kr/learn/courses/30/lessons/86491/questions)

...
| 파일 | 접근 | 복잡도 | 용도 |
|---|---|---|---|
| [효율적인 권장 풀이](./recommended.py) | 수포자별 주기 패턴을 나머지 인덱스로 비교 | `O(n) time, O(1) extra space` | 실전 제출의 기준 풀이 |
| [공개 풀이 접근(재작성)](./alternative.py) | 정답을 한 번 순회하며 세 패턴 점수를 동시에 누적 | `O(n) time, O(1) space` | 같은 문제를 다른 상태 표현으로 비교하는 풀이 |

## 풀이별 해설

### `recommended.py` — 효율적인 권장 풀이

- 핵심 사고: 수포자별 주기 패턴을 나머지 인덱스로 비교
...
- 핵심 사고: 정답을 한 번 순회하며 세 패턴 점수를 동시에 누적
- 복잡도: `O(n) time, O(1) space`
- 적용 범위: 문제의 최대 제한에서도 실행 가능한 대안
- 문제별 주의: 최고점 동점자는 모두 반환해야 하며 번호 순서는 오름차순이다.
...
- [프로그래머스 문제별 풀이 Q&A](https://school.programmers.co.kr/learn/courses/30/lessons/42840/questions)

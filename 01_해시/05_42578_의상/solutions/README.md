...
| 파일 | 접근 | 복잡도 | 용도 |
|---|---|---|---|
| [효율적인 권장 풀이](./recommended.py) | 종류별 (선택+미선택) 경우를 곱함 | `O(n) time, O(n) space` | 실전 제출의 기준 풀이 |
| [공개 풀이 접근(재작성)](./alternative.py) | 종류를 하나씩 추가하는 1상태 DP | `O(n) time, O(n) space` | 같은 문제를 다른 상태 표현으로 비교하는 풀이 |

## 풀이별 해설

### `recommended.py` — 효율적인 권장 풀이

- 핵심 사고: 종류별 (선택+미선택) 경우를 곱함
...
- 핵심 사고: 종류를 하나씩 추가하는 1상태 DP
- 복잡도: `O(n) time, O(n) space`
- 적용 범위: 문제의 최대 제한에서도 실행 가능한 대안
- 문제별 주의: 종류별로 하나만 입을 수 있으며 마지막에 빈 선택을 반드시 제외한다.
...
- [프로그래머스 문제별 풀이 Q&A](https://school.programmers.co.kr/learn/courses/30/lessons/42578/questions)

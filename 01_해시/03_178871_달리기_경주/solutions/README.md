...
| 파일 | 접근 | 복잡도 | 용도 |
|---|---|---|---|
| [효율적인 권장 풀이](./recommended.py) | 이름-순위 역인덱스로 인접 교환 | `O(n+m) time, O(n) space` | 실전 제출의 기준 풀이 |
| [공개 풀이 접근(재작성)](./alternative.py) | 두 방향 매핑을 명시적으로 동기화 | `O(n+m) time, O(n) space` | 같은 문제를 다른 상태 표현으로 비교하는 풀이 |

## 풀이별 해설

### `recommended.py` — 효율적인 권장 풀이

- 핵심 사고: 이름-순위 역인덱스로 인접 교환
...
- 핵심 사고: 두 방향 매핑을 명시적으로 동기화
- 복잡도: `O(n+m) time, O(n) space`
- 적용 범위: 문제의 최대 제한에서도 실행 가능한 대안
- 문제별 주의: 배열만 갱신하거나 위치표만 갱신하면 다음 호출부터 상태가 어긋난다.
...
- [프로그래머스 문제별 풀이 Q&A](https://school.programmers.co.kr/learn/courses/30/lessons/178871/questions)

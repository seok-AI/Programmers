...
| 파일 | 접근 | 복잡도 | 용도 |
|---|---|---|---|
| [효율적인 권장 풀이](./recommended.py) | 장르 집계 후 장르/곡 다중 정렬 | `O(n log n) time, O(n) space` | 실전 제출의 기준 풀이 |
| [공개 풀이 접근(재작성)](./alternative.py) | 전체 인덱스를 장르 총합/곡 재생수로 한 번 정렬 | `O(n log n) time, O(n) space` | 같은 문제를 다른 상태 표현으로 비교하는 풀이 |

## 풀이별 해설

### `recommended.py` — 효율적인 권장 풀이

- 핵심 사고: 장르 집계 후 장르/곡 다중 정렬
...
- 핵심 사고: 전체 인덱스를 장르 총합/곡 재생수로 한 번 정렬
- 복잡도: `O(n log n) time, O(n) space`
- 적용 범위: 문제의 최대 제한에서도 실행 가능한 대안
- 문제별 주의: 곡 자체의 재생 수와 장르 전체 재생 수를 같은 정렬 키로 혼동하지 않는다.
...
- [프로그래머스 문제별 풀이 Q&A](https://school.programmers.co.kr/learn/courses/30/lessons/42579/questions)

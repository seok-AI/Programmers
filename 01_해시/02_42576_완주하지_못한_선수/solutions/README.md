...
| 파일 | 접근 | 복잡도 | 용도 |
|---|---|---|---|
| [효율적인 권장 풀이](./recommended.py) | Counter로 참가/완주 빈도 차감 | `O(n) time, O(n) space` | 실전 제출의 기준 풀이 |
| [공개 풀이 접근(재작성)](./alternative.py) | 두 목록 정렬 후 첫 불일치 탐색 | `O(n log n) time, O(n) space` | 같은 문제를 다른 상태 표현으로 비교하는 풀이 |

## 풀이별 해설

### `recommended.py` — 효율적인 권장 풀이

- 핵심 사고: Counter로 참가/완주 빈도 차감
...
- 핵심 사고: 두 목록 정렬 후 첫 불일치 탐색
- 복잡도: `O(n log n) time, O(n) space`
- 적용 범위: 문제의 최대 제한에서도 실행 가능한 대안
- 문제별 주의: 동명이인이 있으므로 set 차집합을 사용하면 정보가 사라진다.
...
- [프로그래머스 문제별 풀이 Q&A](https://school.programmers.co.kr/learn/courses/30/lessons/42576/questions)

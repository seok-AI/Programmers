...
| 파일 | 접근 | 복잡도 | 용도 |
|---|---|---|---|
| [효율적인 권장 풀이](./recommended.py) | 정렬 후 인접 전화번호만 비교 | `O(n log n) time, O(n) space` | 실전 제출의 기준 풀이 |
| [공개 풀이 접근(재작성)](./alternative.py) | 각 번호의 모든 진접두어를 해시 조회 | `O(total digits) time, O(n) space` | 같은 문제를 다른 상태 표현으로 비교하는 풀이 |

## 풀이별 해설

### `recommended.py` — 효율적인 권장 풀이

- 핵심 사고: 정렬 후 인접 전화번호만 비교
...
- 핵심 사고: 각 번호의 모든 진접두어를 해시 조회
- 복잡도: `O(total digits) time, O(n) space`
- 적용 범위: 문제의 최대 제한에서도 실행 가능한 대안
- 문제별 주의: 문자열 길이순 정렬만으로는 접두어 후보가 인접한다는 보장이 없다.
...
- [프로그래머스 문제별 풀이 Q&A](https://school.programmers.co.kr/learn/courses/30/lessons/42577/questions)

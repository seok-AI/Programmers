...
| 파일 | 접근 | 복잡도 | 용도 |
|---|---|---|---|
| [효율적인 권장 풀이](./recommended.py) | 명령마다 경로 전체의 경계와 장애물을 확인한 뒤 이동 | `O(routes*distance) time, O(1) space` | 실전 제출의 기준 풀이 |
| [공개 풀이 접근(재작성)](./alternative.py) | 후보 도착점까지 한 칸씩 검사하고 실패 시 원위치 | `O(routes*distance) time, O(1) space` | 같은 문제를 다른 상태 표현으로 비교하는 풀이 |

## 풀이별 해설

### `recommended.py` — 효율적인 권장 풀이

- 핵심 사고: 명령마다 경로 전체의 경계와 장애물을 확인한 뒤 이동
...
- 핵심 사고: 후보 도착점까지 한 칸씩 검사하고 실패 시 원위치
- 복잡도: `O(routes*distance) time, O(1) space`
- 적용 범위: 문제의 최대 제한에서도 실행 가능한 대안
- 문제별 주의: 목적지만 확인하면 경로 중간의 장애물을 통과하는 잘못된 이동이 된다.
...
- [프로그래머스 문제별 풀이 Q&A](https://school.programmers.co.kr/learn/courses/30/lessons/172928/questions)

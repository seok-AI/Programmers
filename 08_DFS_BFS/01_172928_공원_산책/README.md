...
- 함수: `solution(park, routes)`
- 포함된 테스트: 공식 공개 예제 3개 + 자체 경계 사례 2개

## 이 유형에 넣은 이유

격자 경계와 장애물을 다루는 입문용 재분류입니다. 실제 핵심은 DFS/BFS가 아니라
명령 시뮬레이션이므로 `게임 맵 최단거리`부터 탐색의 대표 신호를 익히세요.
...
```bash
python3 tools/judge.py "08_DFS_BFS/01_172928_공원_산책"
```

채점기는 `park`, `routes` 순서로 함수를 호출합니다. `tests.json`은 공개 예제,
...
```bash
python3 "08_DFS_BFS/01_172928_공원_산책/solution.py"
```

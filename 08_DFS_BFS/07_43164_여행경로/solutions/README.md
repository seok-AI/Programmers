...
| 파일 | 접근 | 복잡도 | 용도 |
|---|---|---|---|
| [효율적인 권장 풀이](./recommended.py) | 도착지를 최소 힙으로 관리하는 Hierholzer 오일러 경로 | `O(e log e) time, O(e) space` | 실전 제출의 기준 풀이 |
| [공개 풀이 접근(재작성)](./alternative.py) | 도착지를 역정렬한 스택으로 관리하는 Hierholzer 경로 | `O(e log e) time, O(e) space` | 같은 문제를 다른 상태 표현으로 비교하는 풀이 |

## 풀이별 해설

### `recommended.py` — 효율적인 권장 풀이

- 핵심 사고: 도착지를 최소 힙으로 관리하는 Hierholzer 오일러 경로
...
- 핵심 사고: 도착지를 역정렬한 스택으로 관리하는 Hierholzer 경로
- 복잡도: `O(e log e) time, O(e) space`
- 적용 범위: 문제의 최대 제한에서도 실행 가능한 대안
- 문제별 주의: 사전순 DFS에서 처음 완성된 경로를 찾는 것과 Hierholzer의 역방향 확정을 혼동하지 않는다.
...
- [프로그래머스 문제별 풀이 Q&A](https://school.programmers.co.kr/learn/courses/30/lessons/43164/questions)

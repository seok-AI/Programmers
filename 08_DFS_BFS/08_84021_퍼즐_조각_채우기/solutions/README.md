...
| 파일 | 접근 | 복잡도 | 용도 |
|---|---|---|---|
| [효율적인 권장 풀이](./recommended.py) | 빈칸/조각 컴포넌트를 추출하고 회전 정규화해 매칭 | `O(n^2) time, O(n^2) space` | 실전 제출의 기준 풀이 |
| [공개 풀이 접근(재작성)](./alternative.py) | 각 도형의 네 회전 중 사전순 최소 좌표를 표준 모양으로 사용 | `O(n^2) time, O(n^2) space` | 같은 문제를 다른 상태 표현으로 비교하는 풀이 |

## 풀이별 해설

### `recommended.py` — 효율적인 권장 풀이

- 핵심 사고: 빈칸/조각 컴포넌트를 추출하고 회전 정규화해 매칭
...
- 핵심 사고: 각 도형의 네 회전 중 사전순 최소 좌표를 표준 모양으로 사용
- 복잡도: `O(n^2) time, O(n^2) space`
- 적용 범위: 문제의 최대 제한에서도 실행 가능한 대안
- 문제별 주의: 회전할 때마다 음수 좌표가 생기므로 다시 원점 이동·정렬한 뒤 비교해야 한다.
...
- [프로그래머스 문제별 풀이 Q&A](https://school.programmers.co.kr/learn/courses/30/lessons/84021/questions)

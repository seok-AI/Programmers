...
| 파일 | 접근 | 복잡도 | 용도 |
|---|---|---|---|
| [효율적인 권장 풀이](./recommended.py) | 좌표를 2배 확대해 사각형 테두리만 남긴 뒤 BFS | `O(C^2) time, O(C^2) space` | 실전 제출의 기준 풀이 |
| [공개 풀이 접근(재작성)](./alternative.py) | 각 격자점이 합성 도형의 외곽선인지 직접 판정 후 BFS | `O(C^2*r) time, O(C^2) space` | 같은 문제를 다른 상태 표현으로 비교하는 풀이 |

## 풀이별 해설

### `recommended.py` — 효율적인 권장 풀이

- 핵심 사고: 좌표를 2배 확대해 사각형 테두리만 남긴 뒤 BFS
...
- 핵심 사고: 각 격자점이 합성 도형의 외곽선인지 직접 판정 후 BFS
- 복잡도: `O(C^2*r) time, O(C^2) space`
- 적용 범위: 문제의 최대 제한에서도 실행 가능한 대안
- 문제별 주의: 좌표를 확대하지 않으면 모서리가 대각선으로 맞닿은 곳에서 잘못된 지름길이 생긴다.
...
- [프로그래머스 문제별 풀이 Q&A](https://school.programmers.co.kr/learn/courses/30/lessons/87694/questions)

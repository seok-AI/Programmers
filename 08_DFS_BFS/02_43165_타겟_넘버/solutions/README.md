...
| 파일 | 접근 | 복잡도 | 용도 |
|---|---|---|---|
| [효율적인 권장 풀이](./recommended.py) | 현재 합별 경우의 수를 Counter로 압축 | `O(n*S) time, O(S) space` | 실전 제출의 기준 풀이 |
| [공개 풀이 접근(재작성)](./alternative.py) | 각 수에 +와 -를 붙이는 재귀 DFS | `O(2^n) time, O(n) space` | 같은 문제를 다른 상태 표현으로 비교하는 풀이 |

## 풀이별 해설

### `recommended.py` — 효율적인 권장 풀이

- 핵심 사고: 현재 합별 경우의 수를 Counter로 압축
...
- 핵심 사고: 각 수에 +와 -를 붙이는 재귀 DFS
- 복잡도: `O(2^n) time, O(n) space`
- 적용 범위: 문제의 최대 제한에서도 실행 가능한 대안
- 문제별 주의: 서로 다른 부호 배치가 같은 합을 만들 수 있으므로 합의 존재가 아니라 빈도를 저장한다.
...
- [프로그래머스 문제별 풀이 Q&A](https://school.programmers.co.kr/learn/courses/30/lessons/43165/questions)

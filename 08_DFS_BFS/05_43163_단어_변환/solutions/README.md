...
| 파일 | 접근 | 복잡도 | 용도 |
|---|---|---|---|
| [효율적인 권장 풀이](./recommended.py) | 한 글자 차이인 단어를 BFS로 탐색 | `O(n^2*L) time, O(n) space` | 실전 제출의 기준 풀이 |
| [공개 풀이 접근(재작성)](./alternative.py) | 단어 그래프를 미리 만든 뒤 최단거리 BFS | `O(n^2*L) time, O(n^2) space` | 같은 문제를 다른 상태 표현으로 비교하는 풀이 |

## 풀이별 해설

### `recommended.py` — 효율적인 권장 풀이

- 핵심 사고: 한 글자 차이인 단어를 BFS로 탐색
...
- 핵심 사고: 단어 그래프를 미리 만든 뒤 최단거리 BFS
- 복잡도: `O(n^2*L) time, O(n^2) space`
- 적용 범위: 문제의 최대 제한에서도 실행 가능한 대안
- 문제별 주의: target이 words에 없으면 변환할 수 없고, 차이 글자 수는 정확히 1이어야 한다.
...
- [프로그래머스 문제별 풀이 Q&A](https://school.programmers.co.kr/learn/courses/30/lessons/43163/questions)

...
| 파일 | 접근 | 복잡도 | 용도 |
|---|---|---|---|
| [효율적인 권장 풀이](./recommended.py) | 시작점부터 레벨 순서 BFS로 최단거리 탐색 | `O(n*m) time, O(n*m) space` | 실전 제출의 기준 풀이 |
| [공개 풀이 접근(재작성)](./alternative.py) | 거리 배열을 방문 표시로 겸용하는 BFS | `O(n*m) time, O(n*m) space` | 같은 문제를 다른 상태 표현으로 비교하는 풀이 |

## 풀이별 해설

### `recommended.py` — 효율적인 권장 풀이

- 핵심 사고: 시작점부터 레벨 순서 BFS로 최단거리 탐색
...
- 핵심 사고: 거리 배열을 방문 표시로 겸용하는 BFS
- 복잡도: `O(n*m) time, O(n*m) space`
- 적용 범위: 문제의 최대 제한에서도 실행 가능한 대안
- 문제별 주의: 칸 수를 세므로 시작 위치의 거리는 0이 아니라 1이다.
...
- [프로그래머스 문제별 풀이 Q&A](https://school.programmers.co.kr/learn/courses/30/lessons/1844/questions)

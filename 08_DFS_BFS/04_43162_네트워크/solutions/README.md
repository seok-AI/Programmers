...
| 파일 | 접근 | 복잡도 | 용도 |
|---|---|---|---|
| [효율적인 권장 풀이](./recommended.py) | 방문하지 않은 컴퓨터마다 DFS를 시작해 연결요소 계산 | `O(n^2) time, O(n) space` | 실전 제출의 기준 풀이 |
| [공개 풀이 접근(재작성)](./alternative.py) | 연결 행렬의 간선을 Union-Find로 병합 | `O(n^2 alpha(n)) time, O(n) space` | 같은 문제를 다른 상태 표현으로 비교하는 풀이 |

## 풀이별 해설

### `recommended.py` — 효율적인 권장 풀이

- 핵심 사고: 방문하지 않은 컴퓨터마다 DFS를 시작해 연결요소 계산
...
- 핵심 사고: 연결 행렬의 간선을 Union-Find로 병합
- 복잡도: `O(n^2 alpha(n)) time, O(n) space`
- 적용 범위: 문제의 최대 제한에서도 실행 가능한 대안
- 문제별 주의: 연결 행렬의 자기 자신 1은 이미 방문 처리한 뒤 보므로 별도 예외가 필요 없다.
...
- [프로그래머스 문제별 풀이 Q&A](https://school.programmers.co.kr/learn/courses/30/lessons/43162/questions)

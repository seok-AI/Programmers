...
| 파일 | 접근 | 복잡도 | 용도 |
|---|---|---|---|
| [효율적인 권장 풀이](./recommended.py) | 상하 이동 합과 연속 A 구간별 최소 좌우 이동 결합 | `O(n^2) time, O(1) space` | 실전 제출의 기준 풀이 |
| [공개 풀이 접근(재작성)](./alternative.py) | 바꿔야 할 위치 사이의 가장 긴 미방문 A 구간을 조사 | `O(n^2) time, O(1) space` | 같은 문제를 다른 상태 표현으로 비교하는 풀이 |

## 풀이별 해설

### `recommended.py` — 효율적인 권장 풀이

- 핵심 사고: 상하 이동 합과 연속 A 구간별 최소 좌우 이동 결합
...
- 핵심 사고: 바꿔야 할 위치 사이의 가장 긴 미방문 A 구간을 조사
- 복잡도: `O(n^2) time, O(1) space`
- 적용 범위: 문제의 최대 제한에서도 실행 가능한 대안
- 문제별 주의: 단순히 오른쪽으로 끝까지 가면 긴 A 구간을 되돌아 피하는 최적 경로를 놓친다.
...
- [프로그래머스 문제별 풀이 Q&A](https://school.programmers.co.kr/learn/courses/30/lessons/42860/questions)

...
| 파일 | 접근 | 복잡도 | 용도 |
|---|---|---|---|
| [효율적인 권장 풀이](./recommended.py) | 중복 상태 정리 후 앞번호부터 왼쪽 이웃 우선 대여 | `O(n) time, O(n) space` | 실전 제출의 기준 풀이 |
| [공개 풀이 접근(재작성)](./alternative.py) | 학생별 체육복 수 배열에서 왼쪽부터 부족분 전달 | `O(n) time, O(n) space` | 같은 문제를 다른 상태 표현으로 비교하는 풀이 |

## 풀이별 해설

### `recommended.py` — 효율적인 권장 풀이

- 핵심 사고: 중복 상태 정리 후 앞번호부터 왼쪽 이웃 우선 대여
...
- 핵심 사고: 학생별 체육복 수 배열에서 왼쪽부터 부족분 전달
- 복잡도: `O(n) time, O(n) space`
- 적용 범위: 문제의 최대 제한에서도 실행 가능한 대안
- 문제별 주의: 자기 여벌을 먼저 반영하지 않으면 같은 체육복을 두 번 빌려주는 오류가 난다.
...
- [프로그래머스 문제별 풀이 Q&A](https://school.programmers.co.kr/learn/courses/30/lessons/42862/questions)

...
| 파일 | 접근 | 복잡도 | 용도 |
|---|---|---|---|
| [효율적인 권장 풀이](./recommended.py) | 점수를 내림차순 정렬해 상자별 최솟값 선택 | `O(n log n) time, O(n) space` | 실전 제출의 기준 풀이 |
| [공개 풀이 접근(재작성)](./alternative.py) | 1..k 점수 빈도로 높은 점수부터 상자 채우기 | `O(n+k) time, O(k) space` | 같은 문제를 다른 상태 표현으로 비교하는 풀이 |

## 풀이별 해설

### `recommended.py` — 효율적인 권장 풀이

- 핵심 사고: 점수를 내림차순 정렬해 상자별 최솟값 선택
...
- 핵심 사고: 1..k 점수 빈도로 높은 점수부터 상자 채우기
- 복잡도: `O(n+k) time, O(k) space`
- 적용 범위: 문제의 최대 제한에서도 실행 가능한 대안
- 문제별 주의: m개 미만으로 남은 사과는 판매할 수 없고 k는 가격 계산에 직접 필요하지 않다.
...
- [프로그래머스 문제별 풀이 Q&A](https://school.programmers.co.kr/learn/courses/30/lessons/135808/questions)

...
| 파일 | 접근 | 복잡도 | 용도 |
|---|---|---|---|
| [효율적인 권장 풀이](./recommended.py) | 작은 앞자리 숫자를 제거하는 단조 감소 스택 | `O(n) time, O(n) space` | 실전 제출의 기준 풀이 |
| [공개 풀이 접근(재작성)](./alternative.py) | 고정 배열과 top 포인터로 단조 스택 구현 | `O(n) time, O(n) space` | 같은 문제를 다른 상태 표현으로 비교하는 풀이 |

## 풀이별 해설

### `recommended.py` — 효율적인 권장 풀이

- 핵심 사고: 작은 앞자리 숫자를 제거하는 단조 감소 스택
...
- 핵심 사고: 고정 배열과 top 포인터로 단조 스택 구현
- 복잡도: `O(n) time, O(n) space`
- 적용 범위: 문제의 최대 제한에서도 실행 가능한 대안
- 문제별 주의: 숫자가 비내림차순이 아니면 k가 남을 수 있으므로 마지막 뒤쪽 삭제가 필요하다.
...
- [프로그래머스 문제별 풀이 Q&A](https://school.programmers.co.kr/learn/courses/30/lessons/42883/questions)

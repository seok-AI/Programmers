...
| 파일 | 접근 | 복잡도 | 용도 |
|---|---|---|---|
| [효율적인 권장 풀이](./recommended.py) | 가벼운 사람과 무거운 사람을 투 포인터로 짝지음 | `O(n log n) time, O(n) space` | 실전 제출의 기준 풀이 |
| [공개 풀이 접근(재작성)](./alternative.py) | 정렬 덱의 양끝에서 탑승자를 꺼냄 | `O(n log n) time, O(n) space` | 같은 문제를 다른 상태 표현으로 비교하는 풀이 |

## 풀이별 해설

### `recommended.py` — 효율적인 권장 풀이

- 핵심 사고: 가벼운 사람과 무거운 사람을 투 포인터로 짝지음
...
- 핵심 사고: 정렬 덱의 양끝에서 탑승자를 꺼냄
- 복잡도: `O(n log n) time, O(n) space`
- 적용 범위: 문제의 최대 제한에서도 실행 가능한 대안
- 문제별 주의: 한 보트에는 최대 두 명만 탈 수 있다는 조건을 놓치지 않는다.
...
- [프로그래머스 문제별 풀이 Q&A](https://school.programmers.co.kr/learn/courses/30/lessons/42885/questions)

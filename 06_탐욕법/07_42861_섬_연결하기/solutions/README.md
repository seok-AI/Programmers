...
| 파일 | 접근 | 복잡도 | 용도 |
|---|---|---|---|
| [효율적인 권장 풀이](./recommended.py) | 비용순 간선과 Union-Find를 쓰는 Kruskal MST | `O(e log e) time, O(n) space` | 실전 제출의 기준 풀이 |
| [공개 풀이 접근(재작성)](./alternative.py) | 현재 트리와 연결되는 최소 간선을 고르는 Prim MST | `O(e log e) time, O(n+e) space` | 같은 문제를 다른 상태 표현으로 비교하는 풀이 |

## 풀이별 해설

### `recommended.py` — 효율적인 권장 풀이

- 핵심 사고: 비용순 간선과 Union-Find를 쓰는 Kruskal MST
...
- 핵심 사고: 현재 트리와 연결되는 최소 간선을 고르는 Prim MST
- 복잡도: `O(e log e) time, O(n+e) space`
- 적용 범위: 문제의 최대 제한에서도 실행 가능한 대안
- 문제별 주의: 대표가 같은 두 정점을 잇는 간선은 사이클을 만들므로 비용에 포함하지 않는다.
...
- [프로그래머스 문제별 풀이 Q&A](https://school.programmers.co.kr/learn/courses/30/lessons/42861/questions)

...
| 파일 | 접근 | 복잡도 | 용도 |
|---|---|---|---|
| [효율적인 권장 풀이](./recommended.py) | 진출 지점이 빠른 차량부터 그 지점에 카메라 설치 | `O(n log n) time, O(1) extra space` | 실전 제출의 기준 풀이 |
| [공개 풀이 접근(재작성)](./alternative.py) | 겹치는 차량 구간의 교집합을 유지 | `O(n log n) time, O(1) extra space` | 같은 문제를 다른 상태 표현으로 비교하는 풀이 |

## 풀이별 해설

### `recommended.py` — 효율적인 권장 풀이

- 핵심 사고: 진출 지점이 빠른 차량부터 그 지점에 카메라 설치
...
- 핵심 사고: 겹치는 차량 구간의 교집합을 유지
- 복잡도: `O(n log n) time, O(1) extra space`
- 적용 범위: 문제의 최대 제한에서도 실행 가능한 대안
- 문제별 주의: 경계에서 만나는 것도 감시되므로 새 카메라 조건은 진입점이 기존 위치보다 클 때이다.
...
- [프로그래머스 문제별 풀이 Q&A](https://school.programmers.co.kr/learn/courses/30/lessons/42884/questions)
